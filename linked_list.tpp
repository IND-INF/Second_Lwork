...
LinkedList<T>::LinkedList(const LinkedList<T> &list){
    ...
    for(size_t number = 0; number < list.length; number++){
...
const T& LinkedList<T>::Get(size_t index) const{
    if(index >= length) throw ElementIndexOutOfRange(index, length);
    if(index > (length-1)/2){

        Node *current = lastN;

        for(size_t dist = length-1 - index; dist > 0; dist--){
        ...
        for(size_t dist = index; dist > 0; dist--){
            current = current->subs;
        }
...
LinkedList<T>* LinkedList<T>::GetSubList(size_t startIndex, size_t endIndex) const{
    ...
    if(startIndex >= length) throw ElementIndexOutOfRange(startIndex, length);
    if(endIndex > length) throw IndexOutOfRange(endIndex, length);
    ...
    if(endIndex > startIndex){
        ...
        Node *current = firstN;
        for(size_t dist = startIndex; dist > 0; dist--){
            current = current->subs;
        }

        for(size_t number = 0; number < endIndex - startIndex; number++){
...
void LinkedList<T>::InsertAt(const T &item, size_t index){
    ...
    if(index == 0){
        ...
        if(index > (length-1)/2 + 1){
            ...
            for(size_t dist = index - 1; dist > 0; dist--){
