...
template <class T>
DynamicArray<T>::Enumerator::Enumerator(const DynamicArray<T> &dynamicArray, size_t index){

    if(!(dynamicArray.size)) throw EnumeratorCtorError();
    if(index >= dynamicArray.size) throw ElementIndexOutOfRange(index, dynamicArray.size);
...
template <class T>
DynamicArray<T>::DynamicArray(size_t size) : DynamicArray(nullptr, size){}
...
void DynamicArray<T>::Resize(size_t newSize){
    ...
    for(size_t number = 0; number < size && number < newSize; number++){
