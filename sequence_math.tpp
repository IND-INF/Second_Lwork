...
template <class sequenceType, class T>
T SequenceMath::QuickSelect(const sequenceType &sequence, const std::size_t &number){
    ...
    std::size_t lessOrEqualCount = sequence.GetCount() - greaterPart->GetCount();
...
T SequenceMath::Median(const sequenceType &sequence){
    std::size_t number = sequence.GetCount() / 2;
...
template <class sequenceType, class T>
std::size_t SequenceMath::PermutationQuantity(const sequenceType &sequence){
...
seqOfSeqsType* SequenceMath::LessPrevious(const sequenceType &sequence){

    std::size_t number = 0;

    seqOfSeqsType *ans = sequence.template Map<Sequence<T>*,seqOfSeqsType>([&sequence, &number](const T &element){
        ...
        Sequence<T> *subans = subseq->Where([&current](const T &x){ return current > x; });
...
template <class seqOfSeqsType, class sequenceType, class T>
seqOfSeqsType* SequenceMath::Prefixes(const sequenceType &sequence){

    std::size_t number = 0;

    seqOfSeqsType *ans = sequence.template Map<Sequence<T>*,seqOfSeqsType>([&sequence, &number](const T &element){
        number++;
        Sequence<T> *subans = sequence.DirectGetSubsequence(0, number);
        return subans;
    });
...
template <class seqOfSeqsType, class sequenceType, class T>
seqOfSeqsType* SequenceMath::Postfixes(const sequenceType &sequence){

    std::size_t number = 0;

    seqOfSeqsType *ans = sequence.template Map<Sequence<T>*,seqOfSeqsType>([&sequence, &number](const T &element){
        Sequence<T> *subans = sequence.DirectGetSubsequence(number, sequence.GetCount());
        number++;
        return subans;
    });
...
sequenceType* SequenceMath::NeighbourMeans(const sequenceType &sequence){
    ...
    delete nextEnumerator;
    delete prevEnumerator;
...
sequenceType* SequenceMath::QuadraticMeans(const sequenceType &sequence){
    ...
    std::size_t subsumCount = sequence.GetCount() - 1;
    ...
    sequenceType *ans = sequence.template Map<T,sequenceType>([&squareSum, &subsumCount](const T &element){
        return std::sqrt((squareSum - element*element)/subsumCount);
    });
