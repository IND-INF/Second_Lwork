...
IndexOutOfRange::IndexOutOfRange(std::size_t entered, std::size_t minimal, std::size_t maximal) : enteredIndex(entered),
            minimalLimit(minimal), maximalLimit(maximal), message("The entered index is " +
            std::to_string(enteredIndex) +  ", though it can be only between " + std::to_string(minimalLimit) +
            " and " + std::to_string(maximalLimit) + ", inclusive."){}
